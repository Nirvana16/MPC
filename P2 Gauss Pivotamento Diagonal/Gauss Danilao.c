...
// Calculando Gauss - - - - - - - - - - - - - - - - - - - - - - - - - -
   for(i=0; i<=(ordem-1); i++){                                          //Dado na fórmula.
	...
	vetX[ordem-1] = matrizResultante[ordem-1]/(matrizA[ordem-1][ordem-1]);	//Pega o último elemento do Vetor!

	for(i= ordem-2 ; i>=0 ; i--){//Agora a partir da penúltima linha....
		for(j=(ordem -1) ; j>=i ; j--){
			soma += (matrizA[i][j] * vetX[j]);
		}
        vetX[i] = (matrizResultante[i] - soma)/matrizA[i][i];
		soma = 0;
		if(vetX[i] = -0){//Tratamento de erro ao final, por sair 0 negativo!
            vetX[i] = 0;
		}
	}
