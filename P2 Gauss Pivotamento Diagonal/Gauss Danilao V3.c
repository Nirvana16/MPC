...
// Continuação das Variavies, recebendo a ordem informada pelo usuario como ordem da matriz
    ...
    float vetX[ordem], matrizResultante[ordem], matrizResultanteGauss[ordem], matrizAGauss[ordem][ordem];
...
// Passar os valores escalonados da  Matriz Superior para outra individualmente
    for (i=0; i< ordem; i++){
        matrizResultanteGauss[i] = Matriz_superior[i][ordem];  // Recuperando a nova matriz resultante
    }

    for (i=0; i< ordem; i++){
        for (j=0; j<ordem; j++){
            matrizAGauss[i][j] = Matriz_superior[i][j]; // recuperando uma nova matriz A
        }
    }


// Calculando SISTEMA TRIANGULAR --------------------------------------------
    vetX[ordem-1]=matrizResultanteGauss[ordem-1]/matrizAGauss[ordem-1][ordem-1]; // Veja que essa parte do codigo
    for(i=(ordem);i>0;i--){                                                      // eh a mesma do utilizando no sistema triangular
        soma=0;                                                                     //
        for (j=(ordem-1);j>i;j--){                                                  //
            soma+=((matrizAGauss[i][j])*vetX[j]);
		}
		vetX[i]=(((matrizResultanteGauss[i])-soma)/matrizAGauss[i][i]);
	 }
...
//Printar a Matriz A escalonada --------------------------------------------------
	printf("\nMatriz A Escalonada \n");
	for(i=0; i<ordem; i++){
		for(j=0; j<ordem; j++)	{
			printf("|%.1f |",matrizAGauss[i][j]);
		}
		printf("\n");
	}
// Printar Matriz Resultante -----------------------------------------------------
